...
const AI_GRADE_SPEAKING_SCHEMA = {
  ...
  properties: {
    ...
    improvedSample: {
      type: "string",
      description:
        "A realistically improved rewrite of what the student said (same ideas), roughly 0.5–1 band higher — not a Band 9 model. " +
        "Avoid clichéd filler phrases (e.g. \"double-edged sword\", \"two sides of the same coin\", \"in this day and age\", \"last but not least\") — they read as outdated and unnatural, especially spoken aloud.",
    },
  },
  required: ["transcript", "annotations", "notes", "criteria", "overallFeedback", "priorities", "topicVocabulary", "improvedSample"],
};
...
- annotations[]: for errors that anchor to specific words in the transcript — Grammar, Vocabulary, Cohesion, or Idea/Logic (Task Response quality of the answer). Same rules as Writing:
...
- notes[]: ONLY for things that can't be anchored to transcript text — fluency (hesitation, pacing, self-correction), pronunciation, intonation — each with an approximate atSeconds, a category, the IELTS Speaking criterion it evidences, and a comment. Do not duplicate an annotations[] item here.
- criteria[]: one entry per Speaking criterion (FC, LR, GRA, PR), band 1–9 (whole numbers), with a justification.
- overallFeedback: 2–4 sentences to the student.
- priorities: exactly 3 short, concrete action items for next time, ordered by impact — no band numbers.
- topicVocabulary: 5–8 entries relevant to this question's topic, each with meaning (Vietnamese) and one example sentence (English).
- improvedSample: a realistic rewrite of what the student said — keep their ideas and voice, fix language/logic issues, aim for a realistic ~0.5–1 band improvement.
...
- annotations[].comment, notes[].comment, overallFeedback, priorities[] and topicVocabulary[].meaning: VIETNAMESE. Address the student as "em", the teacher voice as "chị". Never "bạn".
