...
async function runAiGrade(submissionId, log = {}) {
  ...
  const aiLog = { ...log, purpose: `grading.${s.kind}`, context: await submissionContext(s) };
  ...
  if (s.kind === "writing") {
    ...
    const { data: raw, model, logId } = await generateJSON({ systemInstruction: SYS_WRITING, prompt, schema: AI_GRADE_JSON_SCHEMA, models, log: aiLog });
