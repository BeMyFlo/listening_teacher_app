...
const SYS_WRITING =
  "You are an experienced IELTS Writing examiner. Grade strictly to the official band descriptors. " +
  "Mark concrete errors and improvements in the student's text and justify each band with evidence. " +
  "The per-criterion justifications (criteria[].comment) stay in English. The inline notes " +
  '(annotations[].comment) and the overall feedback are in Vietnamese, addressing the student as "em" ' +
  'and the teacher voice as "chị" (never "bạn"). ' +
  AI_GRADE_FIELD_SPEC;

const SYS_SPEAKING =
  "You are an experienced IELTS Speaking examiner. Listen to the recording and grade strictly to the " +
...
async function runAiGrade(submissionId) {
  ...
  if (s.kind === "writing") {
    ...
    const raw = await generateJSON({ systemInstruction: SYS_WRITING, prompt, schema: AI_GRADE_JSON_SCHEMA });
    const verr = validateAiGrade(raw, variant);
    if (verr) throw new Error("AI response invalid: " + verr);
    return { kind: "writing", draft: normaliseAiGrade(raw, s.essayText, variant) };
  }
  ...
  if (s.kind === "speaking") {
    ...
    const raw = await generateJSON({ systemInstruction: SYS_SPEAKING, prompt, schema: AI_GRADE_SPEAKING_SCHEMA, audio });
    return { kind: "speaking", draft: normaliseAiSpeaking(raw, variant) };
  }
