...
// Cả 2 lần nộp (nếu có) cho 1 prompt, sắp theo attemptNumber tăng dần.
// attempt1 = bài gốc; attempt2 = bài nộp lại sau Reflection Log (có thể chưa có).
export function promptAttempts(subs, promptId) {
  const rows = subs
    .filter(
      (s) =>
        (s.kind === "writing" || s.kind === "speaking") &&
  ...
  return { attempt1: rows[0] || null, attempt2: rows[1] || null };
}
