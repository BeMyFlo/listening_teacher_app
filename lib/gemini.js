...
async function callOnce({ key, model, parts, schema, systemInstruction, temperature, thinkingBudget }) {
  const url = `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${key}`;
  const generationConfig = { temperature, responseMimeType: "application/json", responseSchema: schema };
  // Giới hạn token "suy nghĩ" — chỉ gửi cho model 2.5 (model đời 3 dùng tham
  // số khác, gửi nhầm có thể bị từ chối 400 mà 400 thì không đổi model).
  if (thinkingBudget != null && /^gemini-2\.5-/.test(model)) {
...
// thinkingBudget (tuỳ chọn): trần token suy nghĩ (model 2.5) — soạn nội dung
//   dài để mặc định thì model nghĩ ~8k token, mất ~45s.
...
async function generateJSON({ systemInstruction, prompt, schema, temperature = 0.2, audio, models, model, log, thinkingBudget }) {
  ...
  for (const m of chain) {
    const t0 = Date.now();
    try {
      const { data, text, usage } = await callOnce({ key, model: m, parts, schema, systemInstruction, temperature, thinkingBudget });
