const { blankIdsFromDoc, sanitizeDoc } = require("./tiptap/doc");
const { mergeNoteBlocks } = require("./tiptap/noteConvert");
const { tenantFilter } = require("./tenant");

// Builder gửi lên `noteBlocks` (mỗi khối 1 khung note/table độc lập) —
// sanitize rồi gộp lại thành noteText/noteDoc như cũ để chấm điểm/hiển thị
// học sinh không cần đổi gì. Section không dùng khối riêng (soạn kiểu cũ,
// hoặc import từ spreadsheet) thì mảng này rỗng — giữ nguyên noteText/noteDoc
// giáo viên đã gửi.
function sanitizeNoteBlocks(blocks) {
  if (!Array.isArray(blocks)) return [];
  return blocks
    .map((b) => ({ noteText: String((b && b.noteText) || ""), noteDoc: sanitizeDoc(b && b.noteDoc) }))
    .filter((b) => b.noteDoc || b.noteText.trim());
}

// Trích các số [[n]] trong noteText — dùng cả khi normalize lẫn validate.
function extractNoteBlankIds(noteText) {
  const ids = [];
  const re = /\[\[(\d+)\]\]/g;
  let m;
  while ((m = re.exec(noteText || ""))) ids.push(Number(m[1]));
  return ids;
}

// Blank ids của 1 section — ưu tiên noteDoc (WYSIWYG), fallback noteText.
function sectionBlankIds(s) {
  if (s.noteDoc && Array.isArray(s.noteDoc.content)) return blankIdsFromDoc(s.noteDoc);
  return extractNoteBlankIds(s.noteText);
}

function normalizeSections(sections) {
  if (!Array.isArray(sections)) return [];
  return sections.map((s) => {
    const noteBlocks = sanitizeNoteBlocks(s.noteBlocks);
    const merged = noteBlocks.length ? mergeNoteBlocks(noteBlocks) : null;
    return {
    name: String(s.name || ""),
    audioId: s.audioId || undefined,
    passageText: String(s.passageText || ""),
    imageId: s.imageId || undefined,
    noteText: merged ? merged.noteText : String(s.noteText || ""),
    noteDoc: merged ? merged.noteDoc : sanitizeDoc(s.noteDoc),
    noteBlocks,
    matchOptions: Array.isArray(s.matchOptions)
      ? s.matchOptions.map((o) => ({ value: String(o.value || ""), label: String(o.label || "") })).filter((o) => o.value)
      : [],
    // Pin vị trí (%) cho câu hỏi Diagram/Map Labelling — bỏ qua entry có
    // fieldId không hợp lệ, kẹp x/y về [0,100] vì tọa độ tính theo % ảnh.
    labelPoints: Array.isArray(s.labelPoints)
      ? s.labelPoints
          .filter((lp) => Number.isFinite(Number(lp.fieldId)))
          .map((lp) => ({
            fieldId: Number(lp.fieldId),
            x: Math.min(100, Math.max(0, Number(lp.x) || 0)),
            y: Math.min(100, Math.max(0, Number(lp.y) || 0))
          }))
      : [],
    fields: Array.isArray(s.fields)
      ? s.fields.map((f) => ({
          id: Number(f.id),
          // Câu "chọn N trong M" gộp nhiều số thứ tự (vd 21-22) vào 1 field —
          // chỉ giữ idEnd khi nó thật sự lớn hơn id (khác thì coi như câu
          // 1 số bình thường).
          idEnd: Number(f.idEnd) > Number(f.id) ? Number(f.idEnd) : null,
          formatLabel: String(f.formatLabel || ""),
          label: String(f.label || ""),
          type: f.type === "choice" ? "choice" : "fill",
          pre: String(f.pre || ""),
          post: String(f.post || ""),
          hint: String(f.hint || ""),
          explanation: String(f.explanation || ""),
          options: Array.isArray(f.options) ? f.options : [],
          selectCount: Math.max(1, Number(f.selectCount) || 1),
          score: Math.max(1, Number(f.score) || 1),
          answers: Array.isArray(f.answers) ? f.answers.map(String) : []
        }))
      : []
    };
  });
}

// Mọi số thứ tự mà 1 field chiếm — bình thường chỉ có id, nhưng câu gộp
// nhiều số (idEnd) chiếm cả dải id..idEnd.
function fieldIdRange(f) {
  const start = Number(f.id);
  const end = Number(f.idEnd) > start ? Number(f.idEnd) : start;
  const out = [];
  for (let i = start; i <= end; i++) out.push(i);
  return out;
}

// Field ids double as DOM row ids and grading-detail keys on the student
// side, so they must be unique across the whole test, not just per section.
function hasDuplicateFieldIds(sections) {
  const ids = sections.flatMap((s) => s.fields.flatMap(fieldIdRange));
  return new Set(ids).size !== ids.length;
}

// Listening sections need audio; Reading sections need a passage and/or an
// image (diagram/map labelling) to be meaningful. Lesson exercises for
// other skills (grammar/vocabulary) have no media requirement.
function sectionMediaError(subject, section) {
  if (subject !== "listening" && subject !== "reading") {
    if (!section.fields.length) {
      return "Each exercise section must have at least one question.";
    }
    return null;
  }
  if (subject === "listening" && !section.audioId) {
    return "Each listening section must have an audio track selected.";
  }
  if (subject === "reading" && !section.passageText.trim() && !section.imageId) {
    return "Each reading section must contain a passage text or a diagram/map image.";
  }
  return null;
}

// Full validation pass for a test's normalized sections: media requirements,
// referenced Audio/Image docs actually exist, "choice" fields have somewhere
// to source their options from, and field ids are unique test-wide.
//
// `ws` bắt buộc: audioId/imageId gửi lên phải thuộc ĐÚNG workspace đang lưu
// bài, không chỉ "tồn tại đâu đó trong toàn hệ thống" — thiếu điều này thì
// một giáo viên workspace A đoán/dò được ObjectId audio của workspace B là
// gắn được audio đó vào bài của mình (PLAN-MULTI-TENANT.md Phase 3 bước 3).
async function validateSections(ws, subject, normalized) {
  const Audio = require("./models/Audio");
  const Image = require("./models/Image");

  for (const s of normalized) {
    const mediaError = sectionMediaError(subject, s);
    if (mediaError) return mediaError;
    if (s.audioId && !(await Audio.exists(tenantFilter(ws, { _id: s.audioId })))) {
      return "Selected audio track does not exist.";
    }
    if (s.imageId && !(await Image.exists(tenantFilter(ws, { _id: s.imageId })))) {
      return "Selected image does not exist.";
    }
    for (const f of s.fields) {
      if (f.type === "choice" && f.options.length === 0 && s.matchOptions.length === 0) {
        return `Question ${f.id}: requires either individual options or shared match options.`;
      }
    }
    {
      const fieldIds = new Set(s.fields.map((f) => f.id));
      const missing = sectionBlankIds(s).filter((id) => !fieldIds.has(id));
      if (missing.length) {
        return `Note completion: chỗ trống [[${missing[0]}]] chưa có câu hỏi/đáp án tương ứng trong danh sách Questions.`;
      }
    }
  }
  if (hasDuplicateFieldIds(normalized)) {
    return "Duplicate question numbers found across sections. Please review question numbers.";
  }
  return null;
}

// Writing/Speaking dùng "prompts" (đề bài, chấm tay) thay vì sections câu
// hỏi — chung shape với Unit.js's PromptSchema (title/instructions/imageId).
function normalizePrompts(prompts) {
  if (!Array.isArray(prompts)) return [];
  return prompts.map((p) => ({
    title: String(p.title || ""),
    instructions: String(p.instructions || ""),
    imageId: p.imageId || undefined,
    // Chỉ dùng cho Writing (rubric Task 1/Task 2); Speaking bỏ qua khi chấm.
    writingTask: ["task1", "task2"].includes(p.writingTask) ? p.writingTask : "task2"
  }));
}

// Mỗi prompt cần tối thiểu tiêu đề + đề bài — ảnh minh hoạ là optional.
// `ws` bắt buộc — cùng lý do đã ghi ở validateSections().
async function validatePrompts(ws, normalized) {
  const Image = require("./models/Image");
  for (const p of normalized) {
    if (!p.title.trim() || !p.instructions.trim()) {
      return "Each prompt needs a title and instructions.";
    }
    if (p.imageId && !(await Image.exists(tenantFilter(ws, { _id: p.imageId })))) {
      return "Selected image does not exist.";
    }
  }
  return null;
}

module.exports = {
  normalizeSections,
  hasDuplicateFieldIds,
  sectionMediaError,
  validateSections,
  normalizePrompts,
  validatePrompts
};
