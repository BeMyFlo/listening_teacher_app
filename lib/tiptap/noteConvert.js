...
// bởi Divider — thường phải thêm 1 Divider dẫn đầu, vì luật parse cũ coi nội
// dung TRƯỚC dấu --- đầu tiên là "intro" hiện ngoài khung.
// NGOẠI LỆ: khối đầu tiên tự nó đã chứa Divider (bài soạn kiểu cũ: đoạn
// hướng dẫn, ---, rồi mới tới các khung) thì KHÔNG chèn — chèn vào sẽ đóng
// hộp luôn đoạn hướng dẫn, làm đổi cách hiển thị bài cũ.
function mergeNoteBlocks(blocks) {
  ...
  const content = [];
  const firstHasDivider = docs[0].content.some((n) => n.type === "horizontalRule");
  if (docs.length > 1 && !firstHasDivider) content.push({ type: "horizontalRule" });
  docs.forEach((d, i) => {
...
module.exports = { noteTextToDoc, docToNoteText, blankIdsFromDoc, mergeNoteBlocks };
