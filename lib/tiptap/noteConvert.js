...
// Tách 1 doc đã gộp (nhiều box cách nhau bởi Divider — cơ chế cũ) thành
// từng khối riêng, để builder hiện lại đúng dạng "mỗi câu 1 khung" cho các
// section soạn từ trước khi có `noteBlocks`. Đoạn "intro" rỗng trước Divider
// đầu tiên (chèn kỹ thuật lúc gộp, xem mergeNoteBlocks) tự bị bỏ qua.
function splitNoteDocIntoBlocks(doc) {
  if (!doc || !Array.isArray(doc.content) || !doc.content.length) return [];
  const parts = [];
  let current = [];
  for (const node of doc.content) {
...
// Gộp nhiều khối note độc lập (soạn riêng trong builder, mỗi khối 1 khung)
// thành 1 doc duy nhất — vẫn lưu vào `noteText`/`noteDoc` như trước để chấm
// điểm/hiển thị học sinh không cần đổi gì. 1 khối = hiện trơn (không đóng
// khung, giống trước giờ); từ 2 khối trở lên = mỗi khối 1 khung, cách nhau
// bởi Divider — cần thêm 1 Divider dẫn đầu vì luật parse cũ coi nội dung
// TRƯỚC dấu --- đầu tiên là "intro" ngoài khung.
function mergeNoteBlocks(blocks) {
  const docs = (blocks || [])
    .map((b) => (b && b.noteDoc && Array.isArray(b.noteDoc.content) ? b.noteDoc : b && b.noteText ? noteTextToDoc(b.noteText) : null))
    .filter((d) => d && Array.isArray(d.content) && d.content.length);
  if (!docs.length) return { noteDoc: null, noteText: "" };
  const content = [];
  if (docs.length > 1) content.push({ type: "horizontalRule" });
  docs.forEach((d, i) => {
    if (i > 0) content.push({ type: "horizontalRule" });
    content.push(...d.content);
  });
  const doc = { type: "doc", content };
  return { noteDoc: doc, noteText: docToNoteText(doc) };
}

module.exports = { noteTextToDoc, docToNoteText, blankIdsFromDoc, splitNoteDocIntoBlocks, mergeNoteBlocks };
