...
async function announceDeadlines(unit, prevDeadlines, { requestedBy, announceAll } = {}) {
  ...
  for (const d of unit.deadlines || []) {
    ...
    if (isSkillLocked(unit, d.classId, d.categoryKey)) continue; // kỹ năng đang khóa cho lớp này
    ...
    while (job.cursor < job.recipientIds.length && Date.now() - t0 < TIME_BUDGET_MS) {
      ...
      for (const studentId of slice) {
        try {
          ...
          await emit({
            studentId,
            // Job đã có workspaceId -> khỏi tra Student cho TỪNG học sinh.
            // Vòng lặp này chạy qua cả lớp nên đây là chỗ N+1 đáng kể nhất.
