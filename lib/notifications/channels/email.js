...
function renderHtml(notif) {
  const rawPath = notif.link || (notif.unitId ? `/student/lessons/${notif.unitId}` : "");
  // Link phải là đường dẫn nội bộ và phải escape trước khi ghép vào href —
  // một dấu nháy kép lọt vào là thoát khỏi attribute.
  const linkPath = /^\/[^"'<>\s]*$/.test(rawPath) ? escapeHtml(rawPath) : "";
  const linkHtml =
    linkPath && APP_URL
      ? `<p style="margin:24px 0 0"><a href="${escapeHtml(APP_URL)}${linkPath}" style="background:#3D97D6;color:#fff;padding:10px 18px;border-radius:6px;text-decoration:none;display:inline-block">Open</a></p>`
      : "";
