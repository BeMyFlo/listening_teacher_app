...
const CHANNEL_CONFIG = {
  ...
  submission_graded: ["inapp"], // chỉ báo chuông cho học sinh
  ticket_update: ["inapp"], // admin cập nhật phiếu hỗ trợ -> chuông cho reporter
};
...
async function emit({ studentId, teacherId, type, dedupeKey, unitId, submissionId, link, title, body, dueAt, workspaceId }) {
  if ((!studentId && !teacherId) || !type || !dedupeKey) {
    throw new Error("emit: (studentId or teacherId), type, dedupeKey are required");
  }

  // Thông báo thuộc workspace của người NHẬN. Chỗ gọi nào cầm sẵn thì truyền
  // vào để đỡ 1 query; không thì tự tra — để không chỗ gọi nào có thể quên
  // (emit() có 6 chỗ gọi, phần lớn không có req.ws trong tầm tay).
