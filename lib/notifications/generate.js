...
async function generateDeadlineNotifications(student) {
  ...
  for (const { unit, categoryKey, dueAt } of dueSoon) {
    ...
    await emit({
      studentId: student._id,
      // Đã cầm sẵn hồ sơ học sinh -> khỏi tra lại mỗi vòng lặp. Hàm này chạy
      // mỗi lần học sinh mở chuông, nên tiết kiệm được 1 query/thông báo.
...
async function generateDeadlineNotificationsForAll() {
  ...
  const students = await Student.find({ classId: { $ne: null } }).select("_id classId name workspaceId").lean();
  let ok = 0;
  let failed = 0;
