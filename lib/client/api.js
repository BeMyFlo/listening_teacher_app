...
import { getTeacherToken, getStudentToken, getAdminToken } from "./session";
...
// Upload đi thẳng từ trình duyệt lên Cloudinary bằng unsigned preset, để không
// đụng trần ~4.5MB của Serverless Function. Hệ quả: cloud name + preset nằm
// trong bundle công khai, ai cũng đọc được.
//
// => Hàng rào thật sự KHÔNG nằm ở đây mà ở Cloudinary console. Với preset
// "ielts_speaking_unsigned" phải bật: Allowed formats, Max file size, và
// Folder cố định. Phía server, mọi URL client gửi lên đều phải qua
// isCloudinaryUrl() trong lib/validate.js và publicId phải suy ra từ URL.
...
async function request(path, { method = "GET", body, auth = false, isForm = false } = {}) {
  const headers = {};
  if (auth) {
    const getter = auth === "student" ? getStudentToken : auth === "admin" ? getAdminToken : getTeacherToken;
    const token = getter();
...
export const api = {
  ...
  student: {
    ...
    notes: {
      list: () => request("/api/student/notes", { auth: "student" }),
      listFor: (params) => {
        const qs = new URLSearchParams(params || {}).toString();
        return request("/api/student/notes" + (qs ? "?" + qs : ""), { auth: "student" });
      },
      create: (body) => request("/api/student/notes", { method: "POST", body, auth: "student" }),
      update: (id, body) =>
        request("/api/student/notes?id=" + id, { method: "PATCH", body, auth: "student" }),
      remove: (id) => request("/api/student/notes?id=" + id, { method: "DELETE", auth: "student" }),
    },
    async uploadSpeakingAudio(blob) {
      // Safari trên iPad/iPhone ghi âm ra audio/mp4, Chrome ra audio/webm.
      // Blob không có tên file nên Cloudinary phải tự đoán định dạng — gắn
      // tên có đuôi đúng để nó nhận diện chính xác (mp3/mp4/webm...).
  ...
  teacher: {
    ...
    regradeUnit: (id) => request("/api/admin/units?id=" + id + "&action=regrade", { method: "POST", auth: "teacher" }),
    ...
    gradingQueue: () => request("/api/admin/grading-queue", { auth: "teacher" }),
    gradeSubmission: (id, d) =>
      request("/api/admin/submissions?id=" + id, { method: "PUT", body: d, auth: "teacher" }),
    aiLessonDraft: (d) => request("/api/admin/ai-lesson", { method: "POST", body: d, auth: "teacher" }),
    ...
    // Per-mock-test submission views. studentId omitted -> per-student
    // overview; studentId given -> full 4-skill breakdown for that student.
    testSubmissions: (testId, studentId) => {
      const qs = new URLSearchParams(
        studentId ? { testId, studentId } : { testId }
      ).toString();
      return request("/api/admin/test-submissions?" + qs, { auth: "teacher" });
    },
  ...
  // Phiếu hỗ trợ — phía người gửi. `role` = "teacher" | "student" (chọn token).
  tickets: {
    mine: (role) => request("/api/tickets", { auth: role }),
    get: (role, id) => request("/api/tickets?id=" + id, { auth: role }),
    create: (role, body) => request("/api/tickets", { method: "POST", body, auth: role }),
    reply: (role, id, body) =>
      request("/api/tickets?id=" + id, { method: "POST", body, auth: role }),
  },

  // Popup "Có gì mới" — 1 lần sau mỗi bản cập nhật. `role` = "teacher" | "student".
  changelog: {
    check: (role) => request("/api/changelog", { auth: role }),
    markSeen: (role) => request("/api/changelog", { method: "POST", auth: role }),
  },

  // Khu quản trị hệ thống (role "admin"). Endpoint dưới /api/sysadmin/*.
  admin: {
    tickets: (params) => {
      const qs = new URLSearchParams(params || {}).toString();
      return request("/api/sysadmin/tickets" + (qs ? "?" + qs : ""), { auth: "admin" });
    },
    getTicket: (id) => request("/api/sysadmin/tickets?id=" + id, { auth: "admin" }),
    updateTicket: (id, body) =>
      request("/api/sysadmin/tickets?id=" + id, { method: "PUT", body, auth: "admin" }),
    dashboard: () => request("/api/sysadmin/dashboard", { auth: "admin" }),
    listUsers: (params) => {
      const qs = new URLSearchParams(params || {}).toString();
      return request("/api/sysadmin/users" + (qs ? "?" + qs : ""), { auth: "admin" });
    },
    createUser: (d) => request("/api/sysadmin/users", { method: "POST", body: d, auth: "admin" }),
    updateUser: (id, d) => request("/api/sysadmin/users?id=" + id, { method: "PUT", body: d, auth: "admin" }),
    deleteUser: (id) => request("/api/sysadmin/users?id=" + id, { method: "DELETE", auth: "admin" }),
    impersonate: (userId) => request("/api/sysadmin/impersonate", { method: "POST", body: { userId }, auth: "admin" }),
    audit: (params) => {
      const qs = new URLSearchParams(params || {}).toString();
      return request("/api/sysadmin/audit" + (qs ? "?" + qs : ""), { auth: "admin" });
    },
    aiLogs: (params) => {
      const qs = new URLSearchParams(params || {}).toString();
      return request("/api/sysadmin/ai-logs" + (qs ? "?" + qs : ""), { auth: "admin" });
    },
    aiLog: (id) => request("/api/sysadmin/ai-logs?id=" + encodeURIComponent(id), { auth: "admin" }),
    notificationsLog: (params) => {
      const qs = new URLSearchParams(params || {}).toString();
      return request("/api/sysadmin/notifications" + (qs ? "?" + qs : ""), { auth: "admin" });
    },
    storage: () => request("/api/sysadmin/storage", { auth: "admin" }),
    system: () => request("/api/sysadmin/system", { auth: "admin" }),
    saveAiModels: (models) => request("/api/sysadmin/system", { method: "PUT", body: { models }, auth: "admin" }),
    saveAiBudget: (budget) => request("/api/sysadmin/system", { method: "PUT", body: { budget }, auth: "admin" }),
    runDeadlineScan: () => request("/api/sysadmin/system", { method: "POST", body: { action: "deadline-scan" }, auth: "admin" }),
    listClasses: () => request("/api/sysadmin/classes", { auth: "admin" }),
    createClass: (d) => request("/api/sysadmin/classes", { method: "POST", body: d, auth: "admin" }),
    updateClass: (id, d) => request("/api/sysadmin/classes?id=" + id, { method: "PUT", body: d, auth: "admin" }),
    deleteClass: (id) => request("/api/sysadmin/classes?id=" + id, { method: "DELETE", auth: "admin" }),
  },
};
