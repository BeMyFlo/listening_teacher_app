  ...
  if (auth) {
    const token =
      auth === "student"
        ? getStudentToken()
        : auth === "any"
        ? getTeacherToken() || getStudentToken()
        : getTeacherToken();
...
export const api = {
  ...
  chat: {
    conversations: () => request("/api/chat/conversations", { auth: "any" }),
    messages: (classId, before) =>
      request("/api/chat/messages?classId=" + classId + (before ? "&before=" + before : ""), { auth: "any" }),
    send: (classId, text, attachments) =>
      request("/api/chat/messages", { method: "POST", body: { classId, text, attachments }, auth: "any" }),
    remove: (id) => request("/api/chat/messages?id=" + id, { method: "DELETE", auth: "any" }),
  },
...
export { request, CLOUDINARY_CLOUD_NAME };
