...
function detailEntry(field, section, value) {
  ...
  return {
    ...
    // Tên dạng bài IELTS (vd "Matching Headings") — dùng để gom thống kê %
    // đúng/sai theo dạng bài cho Reading/Listening.
...
// id câu -> tên dạng bài, đọc trực tiếp từ định nghĩa hiện tại của bài tập/
// skill (không phụ thuộc snapshot cũ trong DB) — để thống kê luôn đúng ngay
// cả với các submission nộp từ trước khi có formatLabel.
function fieldFormatMap(skillBlock) {
  const m = {};
  ((skillBlock && skillBlock.sections) || []).forEach((section) => {
    (section.fields || []).forEach((f) => {
      m[f.id] = f.formatLabel || "Khác";
    });
  });
  return m;
}

// Gom % đúng/sai theo dạng bài từ một danh sách detail đã có formatLabel.
// Sắp % thấp -> cao (dạng yếu nhất lên trước) để giáo viên thấy ngay.
function aggregateByFormat(entries) {
  const m = new Map();
  (entries || []).forEach((d) => {
    const key = d.formatLabel || "Khác";
    if (!m.has(key)) m.set(key, { label: key, correct: 0, total: 0 });
    const o = m.get(key);
    o.total += 1;
    if (d.correct) o.correct += 1;
  });
  return Array.from(m.values())
    .map((o) => ({ ...o, pct: o.total ? Math.round((o.correct / o.total) * 100) : null }))
...
function gradeSubmission(test, submittedAnswers) {
  ...
  // Kỹ năng chưa soạn nội dung -> sections là undefined. Không guard thì
  // forEach ném TypeError và request thành 500 (rebuildDetail bên dưới đã
  // guard sẵn, hai hàm phải nhất quán).
  const sections = (test && Array.isArray(test.sections) ? test.sections : []);
...
module.exports = { normalize, isCorrect, gradeSubmission, rebuildDetail, fieldFormatMap, aggregateByFormat };
