...
const SectionSchema = new mongoose.Schema(
  {
    ...
    // Builder-side: mỗi khối = 1 khung note/table độc lập (soạn riêng, không
    // cần Divider thủ công để tách). Lưu song song với noteText/noteDoc ở
    // trên — 2 field đó luôn là bản GỘP của noteBlocks (xem
    // lib/tiptap/noteConvert.js mergeNoteBlocks), dùng để chấm điểm/hiển thị
    // học sinh không đổi gì; noteBlocks chỉ để builder hiện lại đúng từng
    // khung khi giáo viên mở sửa lại.
    noteBlocks: {
      type: [
        new mongoose.Schema(
          { noteText: { type: String, default: "" }, noteDoc: { type: mongoose.Schema.Types.Mixed, default: null } },
          { _id: false }
        ),
      ],
      default: [],
