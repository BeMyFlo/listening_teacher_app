...
// Không có index nào thì mọi màn hình đều quét toàn collection. Ba index dưới
// phủ đúng các truy vấn đang có:
//   1. "bài của học sinh này, mới nhất trước" — dashboard học sinh, lịch sử nộp.
//   2. chặn nộp lại mock test (pages/api/submissions.js) + tra 1 bài cụ thể.
//   3. hàng chờ chấm của giáo viên.
SubmissionSchema.index({ studentId: 1, submittedAt: -1 });
SubmissionSchema.index({ studentId: 1, testId: 1, testSkill: 1 });
