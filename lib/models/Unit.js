...
// Khóa 1 kỹ năng của Unit cho 1 lớp cụ thể. Có entry (lớp, kỹ năng) = lớp đó
// CHƯA được mở kỹ năng đó: học sinh thấy tab nhưng bị khóa, không vào được,
// không nộp được, không tính vào tiến độ. [] = không khóa gì (tương thích ngược).
const SkillLockSchema = new mongoose.Schema(
  {
    classId: { type: mongoose.Schema.Types.ObjectId, ref: "Class", required: true },
    categoryKey: { type: String, enum: CATEGORY_KEYS, required: true },
  },
  { _id: false }
);
...
const CategorySchema = new mongoose.Schema({
  ...
  theory: {
    // TÊN GÂY HIỂU NHẦM: đây KHÔNG phải HTML thô. Nội dung là markdown rút gọn
    // (cú pháp xem lib/theoryFormat.js) và luôn phải render qua renderTheory(),
    // hàm đó escape trước rồi mới sinh ra tập thẻ cố định. Đừng bao giờ đổ
    // thẳng giá trị này vào dangerouslySetInnerHTML. Giữ tên `html` vì dữ liệu
    // cũ trong DB đang dùng khoá này.
    html: { type: String, default: "" },
    // WYSIWYG source of truth for Theory Content — TipTap/ProseMirror JSON.
    // When present it wins over `html` (kept in sync for legacy readers).
...
const UnitSchema = new mongoose.Schema(
  {
    ...
    // Khóa kỹ năng theo lớp. Xem SkillLockSchema.
    skillLocks: { type: [SkillLockSchema], default: [] },
...
UnitSchema.index({ workspaceId: 1, level: 1, order: 1 });
