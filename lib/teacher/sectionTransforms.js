...
export function emptySection() {
  return { name: "", audioId: "", passageText: "", imageId: "", matchBank: [], noteMode: false, noteText: "", noteDoc: null, fields: [] };
}
...
export function sectionsToPayload(sectionsArr, subject) {
  return sectionsArr.map((sec) => ({
    ...
    // noteDoc là nguồn chính (WYSIWYG); noteText giữ lại để tương thích ngược.
...
export function sectionsToEditor(serverSections) {
  return (serverSections || []).map((s) => ({
    ...
    noteMode: !!((s.noteDoc && s.noteDoc.content && s.noteDoc.content.length) || (s.noteText && s.noteText.trim())),
    noteText: s.noteText || "",
