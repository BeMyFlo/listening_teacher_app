...
const { gradeSubmission, fieldFormatMap, aggregateByFormat } = require("../grade");
const { isSkillLocked } = require("../deadlines");

// Chỉ 2 skill này cần thống kê % theo dạng bài (theo yêu cầu giáo viên).
const FORMAT_STATS_KEYS = ["reading", "listening"];
...
function buildUnitOverview({ unit, submissions, students, classById }) {
  ...
  const rows = students.map((stu) => {
    ...
    const categories = {};
    CATEGORY_KEYS.forEach((key) => {
      // Kỹ năng bị khóa cho lớp của học sinh này -> không tính vào tiến độ.
      if (isSkillLocked(unit, stu.classId, key)) {
        categories[key] = {
          kind: PROMPT_KEYS.includes(key) ? "prompt" : "exercise",
          itemsTotal: 0,
          submitted: 0,
          done: 0,
          graded: 0,
...
function buildStudentDetail({ unit, submissions }) {
  ...
  return CATEGORY_KEYS.map((key) => {
    ...
    const formatStats = FORMAT_STATS_KEYS.includes(key)
      ? aggregateByFormat(exercises.flatMap((e) => e.detail || []))
      : null;
    return { key, label, kind: "exercise", itemsTotal: exercises.length, done, score, total, exercises, formatStats };
  });
}
