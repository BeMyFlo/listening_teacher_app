...
//          cost: { costUsd, priced }, blocked }
// Trả về _id của log (hoặc null).
async function recordAiCall(entry) {
  if (!dbReady()) return null;
  try {
    const { log = {}, systemInstruction, prompt, audio, attempts, model, response, usage, durationMs, error, cost, blocked } = entry;
    let systemHash = "";
    try {
    ...
    const doc = await AiLog.create({
      ...
      costUsd: (cost && cost.costUsd) || 0,
      priced: cost ? cost.priced !== false : true,
      blocked: !!blocked,
