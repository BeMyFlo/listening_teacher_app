...
async function handler(req, res) {
  ...
  const allClasses = await Class.find(tenantFilter(req.ws)).sort({ level: 1, name: 1 }).lean();
  ...
  const [students, units, totalUnits, totalTests, unreadSubmissions] = await Promise.all([
    Student.find(tenantFilter(req.ws, { classId: { $in: classIdList } })).select("name classId").lean(),
    Unit.find(tenantFilter(req.ws, { status: "published", "deadlines.classId": { $in: classIdList } })).lean(),
    Unit.countDocuments(tenantFilter(req.ws)),
    Test.countDocuments(tenantFilter(req.ws)),
    Notification.countDocuments(tenantFilter(req.ws, {
  ...
  const submissions = await Submission.find(tenantFilter(req.ws, { studentId: { $in: studentIds } }))
    .select(
