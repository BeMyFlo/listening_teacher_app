...
// Chấm Speaking: nghe audio + transcript + ghi chú theo mốc giây, và (mới)
// sửa lỗi trực tiếp trên transcript giống Writing (gạch/chèn màu theo nhóm
// lỗi) cho những lỗi neo được vào chữ (Grammar/Vocabulary/Cohesion/Idea).
// Lỗi Fluency/Pronunciation không neo được vào chữ thì vẫn dùng ghi chú theo
// mốc giây bên dưới. AI (Gemini) nghe băng, gỡ transcript và đề xuất cả hai.
...
const CAT_LABEL = {
  ...
  punctuation: "Punctuation", idea: "Idea/Logic", task: "Idea/Logic", style: "Vocabulary", other: "Other",
};
...
export default function SpeakingReview({
  ...
  const [tab, setTab] = useState("notes"); // "notes" | "corrections"
  ...
  return (
    <div className="speaking-review">
      ...
      <div className="ea-modeswitch" style={{ margin: "10px 0" }}>
        <button type="button" className={"rubric-pilltab" + (tab === "notes" ? " active" : "")} onClick={() => setTab("notes")}>
          Delivery notes
        </button>
        <button type="button" className={"rubric-pilltab" + (tab === "corrections" ? " active" : "")} onClick={() => setTab("corrections")}>
          Text corrections
        </button>
      </div>
      ...
      {tab === "corrections" ? (
        !transcript ? (
          <p className="notice info">Fill in the transcript above first (or run AI grade) to mark corrections on it.</p>
        ) : (
          <EssayAnnotator
        ...
        <>
          {notes.length > 0 && (
            <div className="ea-list" style={{ marginTop: 4 }}>
              {notes.map((n) => (
          ...
          <p className="page-sub" style={{ margin: "8px 0 4px" }}>
            Dùng cho lỗi Fluency/Pronunciation không neo được vào chữ — lỗi Grammar/Vocabulary/Cohesion/Idea nên sửa ở tab "Text corrections".
          </p>
